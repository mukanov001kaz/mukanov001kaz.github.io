...
function signUpInfo() {
    if (name.value === "" || email.value === "" || password.value === "") {
        alertText("Введите коректные данные", "signup.html");
        return;
    } else if (!email.value.includes("@") || !email.value.includes(".")) {
        alertText("email должен содержать знак @ и .", "signup.html");
        return;
    } else if (password.value.length < 8) {
        alertText("Пароль должен быть от 8 символов и выше", "signup.html");
        return;
    } else {
        const signUpObj = {
            name: name.value,
